...
def adjust_codes(file_to_adjust):
    ...
    for idx, code in enumerate(codes):
        if idx > 1 and idx < 106:
            old_beef = str(code['Data'])
            mid_beef = old_beef[:69] + '0' + old_beef[70:]
            old_beef = mid_beef

            print "-----------------------------------"
            og_beef = str(code['Data'])
            og_beef = og_beef[57:]
            print og_beef
            print og_beef[0:4]
            print int(og_beef[0:4], 2)
            print og_beef[4:8]
            print int(og_beef[4:8], 2)
            print og_beef[8:12]
            print int(og_beef[8:12], 2)
            print og_beef[12:16]
            print int(og_beef[12:16], 2)
            print og_beef[16:20]
            print int(og_beef[16:20], 2)
            print og_beef[20:24]
            print int(og_beef[20:24], 2)

            og_checksum = (int(og_beef[0:4], 2) + int(og_beef[4:8], 2) + int(og_beef[8:12], 2) + int(og_beef[12:16], 2) + int(og_beef[16:20], 2) + int(og_beef[20:24], 2)) % 16
            og_checksum = format(og_checksum, 'b').zfill(4)
            print og_checksum
            print og_beef[24:28]
            if og_checksum == og_beef[24:28]:
                print "MATCH"
            else:
                print "ERROR"
