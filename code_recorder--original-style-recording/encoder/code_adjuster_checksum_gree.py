...
def adjust_codes(file_to_adjust):
    ...
    for idx, code in enumerate(codes):
        if idx > 0:
            ...
            print(code['Mode'])
            print(old_beef)
            code_data = old_beef[65:129]
            recorded_checksum = old_beef[129:133]
            calculated_checksum = calc_checksum(code_data)
            if str(recorded_checksum) != str(calculated_checksum):
                print('ERROR!!!!!!!!!!!!!!!!!!!')

            new_beef = old_beef[0:94] + '0' + old_beef[95:164] + '0' + old_beef[164:]
            print(new_beef)
            calculated_checksum = calc_checksum(new_beef[65:129])
            new_beef = new_beef[0:129] + calculated_checksum + '4'
            print(new_beef)
            code['Data'] = new_beef
